...
bool AlpineCoreConfig::load(const std::string& filename)
{
    ...
    if (!file.is_open()) {
        xlog::warn("Failed to open RF2 Community Patch (SOPOT) core config: {}", filename);
        return false;
    }
    ...
    xlog::info("Loaded RF2 Community Patch (SOPOT) core config from {}", filename);
    return true;
}
...
void AlpineCoreConfig::save(const std::string& filename) const
{
    ...
    if (!file.is_open()) {
        xlog::warn("Failed to write RF2 Community Patch (SOPOT) core config: {}", filename);
        return;
    }
    ...
    file << "; RF2 Community Patch (SOPOT) System Config File";
    file << "\n\n; This file is automatically generated by RF2 Community Patch (SOPOT).\n";
    file << "; Unlike rf2patch_settings.ini, this file is used by the base game and all mods.\n";
    ...
    file << "AFCCClientVersion=" << PRODUCT_NAME_VERSION << "\n";
    file << "AFCCFileVersion=" << AFCC_VERSION << "\n";

    // Write saved orphaned settings
    if (!orphaned_lines.empty()) {
        file << "\n[OrphanedSettings]\n";
        file << "; Items in this section were unrecognized by your RF2 Community Patch (SOPOT) client.\n";
        file << "; They could be malformed or may require a newer version of RF2 Community Patch (SOPOT).\n";
    ...
    file.close();
    xlog::info("Saved RF2 Community Patch (SOPOT) core config to {}", filename);
}
